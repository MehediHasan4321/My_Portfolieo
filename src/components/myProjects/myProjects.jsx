            ...
            {projects.map((project, index) => <div key={index} className={project?.reverse ? 'container mx-auto py-20 md:flex flex-row-reverse gap-5 md:gap-20' : 'container mx-auto py-20 md:flex gap-5 md:gap-20'}>
                ...
                <div className='space-y-4'>
                    ...
                    <div className='flex gap-4 pt-10'>
                        {
                            project?.links?.map((link, index) => <Link target='_blank' key={index} to={link.link}><button className='px-2 md:px-6 py-2 rounded-xl bg-purple-400 text-white text-sm md:font-semibold hover:bg-purple-500'>{link.name}</button></Link>)
                        }
                    </div>
                </div>
            </div>)}
